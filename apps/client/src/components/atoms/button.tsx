...
>(
  (
    ...
    const renderIcon = () => {
      ...
      // If it's a component type (function, class, or ForwardRef), render it
      ...
      >
        ...
        {Icon && iconPlacement === 'right' && effect === 'expandIcon' && (
          <div className={cn(
            "transition-all duration-200",
            isDisabled
              ? "w-5 translate-x-0 pl-2 opacity-100"
              : "w-0 translate-x-[100%] pl-0 opacity-0 group-hover:w-5 group-hover:translate-x-0 group-hover:pl-2 group-hover:opacity-100"
          )}>
            {renderIcon()}
          </div>
        )}
