...
function MediaPlayerLoading(props: MediaPlayerLoadingProps) {
  ...
  return (
    <LoadingPrimitive
      ...
      {children ?? <Loader2Icon className="text-primary size-20 animate-spin stroke-[.0938rem]" />}
    </LoadingPrimitive>
  );
}
      ...
      {children ?? (
        <div className="flex max-w-md flex-col items-center gap-4 px-6 py-8 text-center">
          ...
          <div className="flex flex-col gap-px text-center">
            <h3 className="text-xl font-semibold tracking-tight">{errorLabel}</h3>
  ...
  return (
    <MediaPlayerTooltip
      ...
      <Button
        ...
        {children ?? <RewindIcon className="size-5!" />}
      </Button>
    </MediaPlayerTooltip>
  );
}
  ...
  return (
    <MediaPlayerTooltip
      ...
      <Button
        ...
        {children ?? <FastForwardIcon className="size-5!" />}
      </Button>
    </MediaPlayerTooltip>
  );
}
      ...
      <SliderPrimitive.Root
        ...
        <SliderPrimitive.Track className="relative h-1 w-full grow overflow-hidden rounded-full bg-white/10">
          <div
            ...
            className="absolute h-full bg-zinc-500 will-change-[width]"
            style={{
              width: `${bufferedProgress * 100}%`,
            }}
          />
          {/* //Played/current progress */}
        ...
        <SliderPrimitive.Thumb className="bg-primary ring-ring/50 relative z-10 block size-3 shrink-0 rounded-full shadow-sm transition-[color,box-shadow] will-change-transform hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50" />
      </SliderPrimitive.Root>
    ...
    <div
      ...
      <MediaPlayerTooltip tooltip="Volume" shortcut="M">
        <Button
          ...
          variant="ghost"
          size="icon"
          className="size-8 bg-none hover:bg-white/10"
          disabled={isDisabled}
          onClick={onMute}>
          {mediaVolumeLevel === 'off' || mediaMuted ? (
            ...
            <Volume1Icon className="size-5! text-white!" />
          )}
        </Button>
      </MediaPlayerTooltip>
      <SliderPrimitive.Root
...
function MediaPlayerTime(props: MediaPlayerTimeProps) {
  ...
  if (variant === 'remaining' || variant === 'duration') {
    return (
      <TimePrimitive
        ...
        dir={context.dir}
        {...timeProps}
        className={cn('text-foreground/80 text-sm tabular-nums', className)}></TimePrimitive>
    );
  }
  ...
  return (
    <TimePrimitive
      ...
      dir={context.dir}
      {...timeProps}
      className={cn('text-foreground/80 flex items-center gap-1 rounded-full p-1', className)}>
      <span className="text-white tabular-nums">{times.current}</span>
      <span
      ...
      <span className="text-white tabular-nums">{times.duration}</span>
    </TimePrimitive>
  );
}

type MediaPlayerPlaybackSpeedProps = React.ComponentProps<typeof DropdownMenuTrigger> &
  ...
  return (
    <Tooltip {...tooltipProps} delayDuration={tooltipDelayDuration}>
      ...
      <TooltipContent
        ...
        className="bg-accent text-foreground flex items-center gap-2 rounded-full border px-2 py-1 font-medium data-[side=top]:mb-3.5 dark:bg-zinc-900 [&>span]:hidden">
        <p>{tooltip}</p>
...
function KeyboardListener() {
  const context = useMediaPlayerContext('KeyboardListener');
  const dispatch = useMediaDispatch();
  const mediaPaused = useMediaSelector((state) => state.mediaPaused ?? true);
  const isFullscreen = useMediaSelector((state) => state.mediaIsFullscreen ?? false);
  const mediaMuted = useMediaSelector((state) => state.mediaMuted ?? false);
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Don't hijack media controls
      if (target.closest('media-player')) return;
  ...
  }, [dispatch, mediaPaused, isFullscreen, mediaMuted, isPictureInPicture, context.mediaRef]);
