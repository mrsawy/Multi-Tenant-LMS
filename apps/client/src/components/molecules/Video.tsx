...
const Video = ({ url, toggleComplete, played, setPlayed, watched, setWatched, isUploadedVideo }: VideoProps) => {
  ...
  return (
    <div dir="ltr" id="video-player-wrapper" className="group relative mb-6 aspect-video overflow-hidden rounded-lg bg-black" onClick={handlePlayPause}>
      {loading && (
      ...
      {isUploadedVideo ? (
        <ReactPlayer ref={playerRef} src={url} width="100%" height="100%" playing={playing} volume={volume} muted={muted} onTimeUpdate={handleTimeChange} playbackRate={playbackRate} onDurationChange={handleDuration} onReady={handleReady} controls={false} />
      ) : (
        <ReactPlayer
          ref={playerRef}
          src={url}
