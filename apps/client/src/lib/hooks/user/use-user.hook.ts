...
import { createUserAction, updateOneUserAction } from '@/lib/actions/user/user.action';
...
export function useUpdateUser() {
  ...
  return useMutation({
    mutationFn: async ({ userId, userData }: { userId: string; userData: EditUserFormData }) => {
      return await updateOneUserAction(userId, userData);
    },
...
export function useDeleteUser() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      return await createAuthorizedNatsRequest('users.organizationDeleteUser', {
        userId,
      });
    },
