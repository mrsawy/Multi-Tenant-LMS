...
export class PaymobService {
  ...
  async getPaymentUrl(initiateSubscriptionDto: InitiateSubscriptionDto) {
    try {
      ...
      if (subscriptionType == SubscriptionType.ORGANIZATION_PLAN) {
        ...
        price =
          frequency == 30 ? savedPlan?.price.MONTHLY : savedPlan?.price.YEARLY;
      } else {
        const foundedCourse = await this.courseService.findOneById(courseId);
      ...
      if (!targetPlan) {
        ...
        targetPlan = await this.createSubscriptionPlan({
  ...
  async createSubscription(
    createPaymobSubscriptionDto: CreatePaymobSubscriptionDto,
  ) {
    try {
      const response = await axios.post<PaymobPaymentIntentionResponse>(
        process.env.BASE_PAYMOB + '/v1/intention/',
  ...
  async createSubscriptionPlan(
    paymobSubscriptionPlan: PaymobSubscriptionPlan & { token: string },
  ) {
    const response = await axios.post<PaymobSubscriptionPlan>(
      process.env.BASE_PAYMOB + '/api/acceptance/subscription-plans',
  ...
  async listAllSubscriptionPlans(
    token: string,
  ...
  async login() {
    const response = await axios.post(
      process.env.BASE_PAYMOB + '/api/auth/tokens',
      {
  ...
  async getTransactionData(id: string) {
    try {
      const token = await this.login();
      const transactionData = await axios.get<PaymobTransaction>(
        process.env.BASE_PAYMOB + '/api/acceptance/transactions/' + id,
        {
          headers: {
            Authorization: 'Bearer ' + token,
          },
  ...
  async webhookSubscription(webhookBody: any) {
    try {
      if (
        !webhookBody.subscription_data ||
        webhookBody.trigger_type !== 'Subscription Created'
      ) {
