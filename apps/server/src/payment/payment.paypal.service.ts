...
import {
  ...
  PayPalWebhookBody,
  PayPalWebhookBodyType,
} from './types/paybalWebhookBody.interface';
...
export class PaypalPaymentService {
  ...
  async getPaymentUrl(
    ...
    try {
      ...
      if (subscriptionType === SubscriptionType.ORGANIZATION_PLAN) {
        ...
        price =
          frequency == 30
            ? subscribableEntity?.price.MONTHLY
            : subscribableEntity?.price.YEARLY;
      } else if (subscriptionType === SubscriptionType.USER_COURSE) {
        subscribableEntity = await this.courseService.findOneById(courseId);
        if (!subscribableEntity)
          throw new NotFoundException('Course Not Found');
        const pricing = this.courseService.getCoursePricing(
          subscribableEntity,
          frequency,
        );
      ...
      if (!subscribableEntity.paypalPlanId) {
        const paypalProduct = await this.createPaypalProduct(
          {
          ...
          billing_cycles: [
            {
              frequency: {
                interval_unit:
                  frequency == 30 ? IntervalUnit.MONTH : IntervalUnit.YEAR,
                interval_count: 1,
              },
              total_cycles: 0,
      ...
      const { country, city, street, country_code } =
        initiateSubscriptionDto.billing_data;
      ...
      const subscriptionData = {
        custom_id: shorten(
          JSON.stringify({
            ...
            userId,
            billingCycle:
              frequency == 30 ? BillingCycle.MONTHLY : BillingCycle.YEARLY,
          }),
        ),
      ...
      if (!links) {
        throw new BadRequestException(
          'No approval link found in the subscription response',
        );
      }
      return {
  ...
  async createSubscription(
    createPaymobSubscriptionDto: CreatePaypalSubscriptionDto,
    token: string,
  ) {
    const headers = {
      'Content-Type': 'application/json',
  ...
  async webhookSubscription(webhookBody: PayPalWebhookBodyType) {
    ...
    if (webhookBody.event_type === 'BILLING.SUBSCRIPTION.ACTIVATED') {
      ...
      const {
        ...
        subscriptionType,
        userId,
      } = JSON.parse(restore(subResource.custom_id));
      const next_billing_time = billing_info?.next_billing_time;
      console.dir(
        { subscriber, billing_info, links, webhookLinks },
        { depth: null },
      );
      const subscription: SubscriptionTypeDef = {
        ...
        billing: {
          ...
          amount: Number(
            (billing_info as PayPalBillingInfo).outstanding_balance.value,
          ),
          currency: (billing_info as PayPalBillingInfo).outstanding_balance
            .currency_code,
          billingCycle:
            frequency == 30 ? BillingCycle.MONTHLY : BillingCycle.YEARLY,
        },
        createdAt: create_time,
        updatedAt: update_time as string,
      };

      if (subscriptionType == SubscriptionType.ORGANIZATION_PLAN) {
        return await this.organizationService.createOrganizationSubscription(
          organizationId,
          planId,
          subscription,
        );
      ...
      if (subscriptionType == SubscriptionType.USER_COURSE) {
        return await this.enrollmentService.enrollUserToCourse(
          userId,
          courseId,
    ...
    if (webhookBody.event_type === 'PAYMENT.SALE.COMPLETED') {
      ...
      const { billing_agreement_id, amount, create_time, update_time } =
        subResource;
      console.dir({ subResource }, { depth: null });

      if (!billing_agreement_id) {
        throw new BadRequestException(
          'No billing agreement id found in the sale resource',
        );
      }

      const subscriptionDetails =
        await this.getSubscriptionDetails(billing_agreement_id);
      const { organizationId, courseId, subscriptionType, userId } = JSON.parse(
        restore(subscriptionDetails.custom_id),
      );
      ...
      const subscription: Partial<SubscriptionTypeDef> = {
        ...
        next_billing: subscriptionDetails.billing_info
          ?.next_billing_time as string,
        reminder_date: subtractDays(
          subscriptionDetails.billing_info?.next_billing_time as string,
          5,
        ),
      ...
      if (subscriptionType == SubscriptionType.ORGANIZATION_PLAN) {
        return await this.organizationService.renewOrganizationSubscription(
          organizationId,
          subscription,
        );
      }
      ...
      if (subscriptionType == SubscriptionType.USER_COURSE) {
        return await this.enrollmentService.updateUserSubscription(
          userId,
          courseId,
          subscription,
        );
  ...
  async getSubscriptionDetails(subscriptionId: string) {
    try {
      ...
      const subscriptionResponse = await axios.get<PayPalSubscriptionResource>(
