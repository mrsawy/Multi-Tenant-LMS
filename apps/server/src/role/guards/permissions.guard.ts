...
export class PermissionsGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        try {
            ...
            throw new InternalServerErrorException(error.message || 'An error occurred while checking permissions');
        }
    }

    // Helper method for checking specific resource instances
    // Use this when you have the actual resource object
    canAccessResource(ability: any, action: string, subject: string, resource: any): boolean {
        try {
